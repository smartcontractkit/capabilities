...
func TestNewLogTriggerService(t *testing.T) {
	...
	t.Run("ok initialize interval", func(t *testing.T) {
		trigger, err := NewLogTriggerService(evmService, store, lggr, beholderProcessor, messageBuilder, 10*time.Second, 0, 0, nil)
		require.NoError(t, err)
	...
	t.Run("ok initialize all params", func(t *testing.T) {
		trigger, err := NewLogTriggerService(evmService, store, lggr, beholderProcessor, messageBuilder, 10*time.Second, 100, 50, nil)
		require.NoError(t, err)
		require.Equal(t, 10*time.Second, trigger.logTriggerPollInterval)
	...
	t.Run("ok initialize buffer only", func(t *testing.T) {
		trigger, err := NewLogTriggerService(evmService, store, lggr, beholderProcessor, messageBuilder, 10*time.Second, 10000, 0, nil)
		require.NoError(t, err)
		require.Equal(t, 10*time.Second, trigger.logTriggerPollInterval)
	...
	t.Run("ok initialize query limit only", func(t *testing.T) {
		trigger, err := NewLogTriggerService(evmService, store, lggr, beholderProcessor, messageBuilder, 10*time.Second, 0, 100, nil)
		require.NoError(t, err)
		require.Equal(t, 10*time.Second, trigger.logTriggerPollInterval)
	...
	t.Run("negative poll interval", func(t *testing.T) {
		_, err := NewLogTriggerService(evmService, store, lggr, beholderProcessor, messageBuilder, -1*time.Second, 0, 0, nil)
		require.Error(t, err)
	...
	t.Run("limit query log size >= send channel buffer size", func(t *testing.T) {
		_, err := NewLogTriggerService(evmService, store, lggr, beholderProcessor, messageBuilder, time.Second, 5, 10, nil)
		require.Error(t, err)
	...
	t.Run("limit query log size >= default send channel buffer size", func(t *testing.T) {
		_, err := NewLogTriggerService(evmService, store, lggr, beholderProcessor, messageBuilder, time.Second, 0, defaultSendChannelBufferSize+1, nil)
		require.Error(t, err)
