...
type Dependencies struct {
	// Connector is the gateway connection. An outbound request is sent over it - the
	// gateway is what actually reaches the internet, whether by fetching on this
	// node's behalf or by tunnelling for it - and it is what signs on this node's
	// behalf.
...
// NewService returns the HTTP action capability, configured and wired to the
...
func NewService(lggr logger.Logger, cfg common.ServiceConfig, deps Dependencies) (*service, error) {
	// Only gateway mode needs one: in direct mode this capability makes its own
	// requests, which is what that mode is.
	if deps.Connector == nil && cfg.ProxyMode == common.ProxyModeGateway {
		return nil, errors.New("the HTTP action capability needs a gateway connector in gateway mode: its requests go out through one")
	}
	...
	if s.client, err = NewOutboundRequestClient(deps.Connector, s.cfg, s.lggr, s.metrics, s.validator); err != nil {
		return nil, err
	}
...
func (s *service) SendRequest(ctx context.Context, metadata capabilities.RequestMetadata, input *protos.Request) (*capabilities.ResponseAndMetadata[*protos.Response], caperrors.Error) {
	...
	responseAndMetadata := capabilities.ResponseAndMetadata[*protos.Response]{
		Response:         response,
...
func NewOutboundRequestClient(gatewayConnector core.GatewayConnector, serviceConfig common.ServiceConfig, lggr logger.Logger, metrics *common.Metrics, validator common.RequestValidator) (common.OutboundRequestClient, error) {
	...
	case common.ProxyModeGateway:
		return NewGatewayOutboundProxy(gatewayConnector, serviceConfig, lggr, metrics, validator)
	default:
