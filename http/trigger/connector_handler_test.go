...
func setupWithTriggerChannelBuffer(t *testing.T, lggr logger.Logger, triggerChBuffer int) (*connectorHandler, *mockGatewayConnector, chan capabilities.TriggerAndId[*protos.Payload], *requestCache) {
	...
	require.NoError(t, err)
	sdkCfg := &protos.Config{
		AuthorizedKeys: []*protos.AuthorizedKey{
			{
				PublicKey: publicKey,
				Type:      protos.KeyType_KEY_TYPE_ECDSA_EVM,
			},
		},
	}
	sendCh := make(chan capabilities.TriggerAndId[*protos.Payload], triggerChBuffer)
...
func setup(t *testing.T, lggr logger.Logger) (*connectorHandler, *mockGatewayConnector, <-chan capabilities.TriggerAndId[*protos.Payload], *requestCache) {
	return setupWithTriggerChannelBuffer(t, lggr, 1)
}

func requireWorkflowTriggered(t *testing.T, triggerCh <-chan capabilities.TriggerAndId[*protos.Payload], req *jsonrpc.Request[json.RawMessage], connector *mockGatewayConnector, handler *connectorHandler, key gateway_common.AuthorizedKey, requestCache *requestCache) {
	...
	go func() {
		...
		case triggerReq := <-triggerCh:
			...
			require.Equal(t, protos.KeyType_KEY_TYPE_ECDSA_EVM, triggerReq.Trigger.Key.Type, "Key type should match")
...
func TestRegisterWorkflow_InvalidECDSAPublicKey(t *testing.T) {
	lggr := logger.Test(t)
	handler, _, _, _ := setup(t, lggr)
	sendCh := make(chan capabilities.TriggerAndId[*protos.Payload], 1)

	testCases := []struct {
		name      string
		publicKey string
		keyType   protos.KeyType
		errorMsg  string
	}{
		{
			name:      "invalid publicKey format (nothex)",
			publicKey: "nothex",
			keyType:   protos.KeyType_KEY_TYPE_ECDSA_EVM,
		...
		},
		{
			...
			publicKey: publicKey,
			keyType:   protos.KeyType_KEY_TYPE_UNSPECIFIED,
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			invalidKey := &protos.AuthorizedKey{
...
func TestHandleGatewayMessage_PullAuthMetadata(t *testing.T) {
	...
	triggerCh2 := make(chan capabilities.TriggerAndId[*protos.Payload], 1)
	selector := gateway_common.WorkflowSelector{
...
func TestHandleGatewayMessage_TriggerFailureDoesNotCacheThenSameRequestSucceeds(t *testing.T) {
	...
	triggerCh <- capabilities.TriggerAndId[*protos.Payload]{
		Id: "prefill-placeholder",
		Trigger: &protos.Payload{
			Input: []byte(`{}`),
			Key: &protos.AuthorizedKey{
				Type:      protos.KeyType_KEY_TYPE_ECDSA_EVM,
				PublicKey: publicKey,
			},
		},
	...
	case triggerReq := <-triggerCh:
		...
		require.Equal(t, protos.KeyType_KEY_TYPE_ECDSA_EVM, triggerReq.Trigger.Key.Type)
