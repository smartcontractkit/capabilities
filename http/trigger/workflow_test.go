...
func TestWorkflow_Trigger_Success(t *testing.T) {
	...
	trigger := capabilities.TriggerAndId[*protos.Payload]{
		Id: "trigger-123",
...
func TestWorkflowStore_GetWorkflows_Multiple(t *testing.T) {
	...
	sendCh1 := make(chan capabilities.TriggerAndId[*protos.Payload], 1)
	sendCh2 := make(chan capabilities.TriggerAndId[*protos.Payload], 1)
	sendCh3 := make(chan capabilities.TriggerAndId[*protos.Payload], 1)
...
func TestWorkflow_Trigger_MultipleSuccessful(t *testing.T) {
	...
	sendCh := make(chan capabilities.TriggerAndId[*protos.Payload], 3) // Buffer of 3
	wf := newWorkflow(workflowSelector, authorizedKeys, sendCh)

	ctx := context.Background()

	trigger1 := capabilities.TriggerAndId[*protos.Payload]{
		Id:      "trigger-1",
		Trigger: &protos.Payload{Input: nil},
	}
	trigger2 := capabilities.TriggerAndId[*protos.Payload]{
		Id:      "trigger-2",
		Trigger: &protos.Payload{Input: nil},
	}
	trigger3 := capabilities.TriggerAndId[*protos.Payload]{
		Id:      "trigger-3",
		Trigger: &protos.Payload{Input: nil},
	}
...
func testWorkflow() (*workflow, chan capabilities.TriggerAndId[*protos.Payload]) {
