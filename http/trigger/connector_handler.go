...
func (h *connectorHandler) RegisterWorkflow(ctx context.Context, input WorkflowRegistrationInput, sendCh chan<- capabilities.TriggerAndId[*protos.Payload]) error {
	if input.Config == nil {
...
func (h *connectorHandler) validateAuthorizedKeys(inputKeys []*protos.AuthorizedKey) ([]gateway_common.AuthorizedKey, error) {
	if len(inputKeys) == 0 {
	...
	for _, key := range inputKeys {
		switch key.Type {
		case protos.KeyType_KEY_TYPE_ECDSA_EVM:
...
func (h *connectorHandler) triggerWorkflow(ctx context.Context, workflowID string, reqID string, gatewayID string, input []byte, key gateway_common.AuthorizedKey) error {
	...
	err := workflow.trigger(ctx, capabilities.TriggerAndId[*protos.Payload]{
