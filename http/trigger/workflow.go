...
type workflow struct {
	...
	sendCh           chan<- capabilities.TriggerAndId[*protos.Payload]
	closed           bool
	metadata         WorkflowRegistrationMetadata
}

func newWorkflow(workflowSelector gateway.WorkflowSelector, authorizedKeys []gateway.AuthorizedKey, sendCh chan<- capabilities.TriggerAndId[*protos.Payload]) *workflow {
...
func newWorkflowWithMetadata(workflowSelector gateway.WorkflowSelector, authorizedKeys []gateway.AuthorizedKey, sendCh chan<- capabilities.TriggerAndId[*protos.Payload], metadata WorkflowRegistrationMetadata) *workflow {
	authorizedKeysMap := make(map[gateway.AuthorizedKey]struct{})
	for _, key := range authorizedKeys {
		authorizedKeysMap[key] = struct{}{}
	}
...
func (w *workflow) trigger(ctx context.Context, trigger capabilities.TriggerAndId[*protos.Payload]) error {
	if trigger.Id == "" {
