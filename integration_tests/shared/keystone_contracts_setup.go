...
func setupCapabilitiesRegistryContract(ctx context.Context, t *testing.T, workflowDon donInfo, triggerDon donInfo,
	...
	mockTarget := kcr.CapabilitiesRegistryCapability{
		LabelledName:   "mock-target",
		Version:        "1.0.0",
		CapabilityType: CapabilityTypeTarget,
	}
	mtid, err := reg.GetHashedCapabilityId(&bind.CallOpts{}, mockTarget.LabelledName, mockTarget.Version)
	require.NoError(t, err)
	...
	for _, wfPeer := range workflowDon.peerIDs {
		...
		n.HashedCapabilityIds = [][32]byte{ocrid, cid, mtid, kvid}
