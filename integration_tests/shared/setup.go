...
func SetupDonsWithTransmissionSchedule(ctx context.Context, t *testing.T, workflowDonInfo donInfo, triggerDonInfo donInfo, targetDonInfo donInfo, cronSchedule string, deltaStage string, schedule string) (*feeds_consumer.KeystoneFeedsConsumer, *mocks.ReportsSink, *mocks.TargetSink) {
	...
	reportsSink := mocks.NewReportsSink()
	targetSink := mocks.NewTargetSink()

	libocr := mocks.NewMockLibOCR(t, workflowDonInfo.F, 1*time.Second)
	workflowDonNodes, _, _ := createDons(ctx, t, lggr, reportsSink,
		workflowDonInfo, triggerDonInfo, targetDonInfo,
		ethBlockchain, capabilitiesRegistryAddr, forwarderAddr,
		workflowDonInfo.keyBundles, transactor, libocr, targetSink)
	for _, node := range workflowDonNodes {
		// TODO: add an argument for which capabilities to use dynamically
		addStandardCapabilityKV(t, node)
		addStandardCapabilityCron(t, node)
		addWorkflowJobCron(t, node, workflowName, workflowOwnerID, cronSchedule, consumerAddr, deltaStage, schedule)
	}
	...
	return consumer, reportsSink, targetSink
}

func createDons(ctx context.Context, t *testing.T, lggr logger.Logger, reportsSink *mocks.ReportsSink,
	workflowDon donInfo,
	triggerDon donInfo,
	targetDon donInfo,
	...
	libocr *mocks.MockLibOCR,
	targetSink *mocks.TargetSink,
) ([]*cltest.TestApplication, []*cltest.TestApplication, []*cltest.TestApplication) {
	broker := mocks.NewTestAsyncMessageBroker(t, 1000)
