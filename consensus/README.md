...
The Report request type is used to create a report that can be sent to the forwarder contract (https://github.com/smartcontractkit/chainlink-evm/blob/develop/contracts/src/v0.8/keystone/KeystoneForwarder.sol).  Each node in the DON 
submits a report and if consensus is reached the metadata required by the forwarder contract is pre-pended
to the report which will then be signed by each node in the DON.  Each node will receive the report with pre-pended metadata
and the signatures from all nodes.
...
by checking that the consensus descriptor and metadata for a given request matches the consensus descriptor and metadata
...
to ensure that the report and signatures are in the correct format for the forwarder contract.
