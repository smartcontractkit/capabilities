...
func Test_Transmit_ReturnsUserError_When_Invalid_Type_For_Median_Aggregation(t *testing.T) {
	lggr := logger.Test(t)
	sendResponseCalled := false
	...
	info := &structpb.Struct{
		Fields: map[string]*structpb.Value{
			plugin.InfoRequestID:               structpb.NewStringValue("test-request-id-failure"),
			plugin.InfoConsensusFailureMessage: structpb.NewStringValue(failureMsg),
			plugin.InfoConsensusFailureCode:    structpb.NewStringValue(oracletypes.ConsensusFailureCode_INVALID_TYPE_FOR_MEDIAN_AGGREGATION.String()),
		},
	}
	infoBytes, err := proto.Marshal(info)
	require.NoError(t, err)
