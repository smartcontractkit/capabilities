...
abstract contract OCR2Base is ConfirmedOwner, OCR2Abstract {
    ...
    function _requireExpectedMsgDataLength(
        ...
        if (msg.data.length != expected)
            revert ReportInvalid("calldata length mismatch");
    }
    ...
    function transmit(
...

        {
            ...
            uint256 expectedNumSignatures = (s_configInfo.n + s_configInfo.f) /
                2 +
                1;

            if (rs.length != expectedNumSignatures)
                revert ReportInvalid("wrong number of signatures");
            if (rs.length != ss.length)
                revert ReportInvalid(
                    "report rs and ss must be of equal length"
                );
...

        {
            ...
            for (uint256 i = 0; i < rs.length; ++i) {
                ...
                if (o.role != Role.Signer)
                    revert ReportInvalid("address not authorized to sign");
                if (signed[o.index] != address(0))
                    revert ReportInvalid("non-unique signature");
