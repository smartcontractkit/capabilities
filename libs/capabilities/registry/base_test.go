...
func TestRegistry_ChecksExecutionAPIByType(t *testing.T) {
	tcs := []struct {
		name          string
		newCapability func(ctx context.Context, reg capabilities.CapabilitiesRegistryBase) (string, error)
		getCapability func(ctx context.Context, reg capabilities.CapabilitiesRegistryBase, id string) error
		errContains   string
	}{
		{
			name: "action",
