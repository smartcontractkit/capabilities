...
var _ SharedPeer = &don2DonSharedPeer{}
...
// PeerSource supplies the started peer this rides on: its group factory and its identity. Both are
// read when this service starts rather than when it is built, because the peer is usually started
// after it - which is also why this is an interface rather than the two values.
type PeerSource interface {
	PeerGroupFactory() networking.PeerGroupFactory
	PeerID() ragetypes.PeerID
}

func NewDon2DonSharedPeer(peerSource PeerSource, bootstrappers []ocrcommontypes.BootstrapperLocator, lggr logger.Logger) *don2DonSharedPeer {
	sp := &don2DonSharedPeer{
		peerSource:      peerSource,
		bootstrappers:   bootstrappers,
		recvCh:          make(chan Message, defaultRecvChSize),
...
func (sp *don2DonSharedPeer) start(ctx context.Context) error {
	...
	if sp.peerSource == nil {
		return errors.New("field PeerSource is not set")
	}
	sp.pgFactory = sp.peerSource.PeerGroupFactory()
	if sp.pgFactory == nil {
		return errors.New("the peer source has no PeerGroupFactory; it was probably not started before Don2DonSharedPeer, or failed to initialize")
	}
	sp.myID = sp.peerSource.PeerID()
	if (sp.myID == ragetypes.PeerID{}) {
		return errors.New("the peer source has no PeerID; was it started before Don2DonSharedPeer?")
	}
...
func (sp *don2DonSharedPeer) Receive() <-chan Message {
	return sp.recvCh
}

func (sp *don2DonSharedPeer) UpdateConnections(peers map[ragetypes.PeerID]StreamConfig) error {
	return errors.New("UpdateConnections is not supported, use UpdateConnectionsByDON instead")
}

func (sp *don2DonSharedPeer) UpdateConnectionsByDONs(ctx context.Context, donPairs []DonPair, streamConfig StreamConfig) error {
...
func (sp *don2DonSharedPeer) updateConnections(donPairs []DonPair, desiredDONPairsIDs map[string]struct{}, desiredRemotePeers map[ragetypes.PeerID]struct{}, streamConfig StreamConfig) error {
	sp.mu.Lock()
	defer sp.mu.Unlock()
...
func (sp *don2DonSharedPeer) recvLoopSingle(ctx context.Context, pid ragetypes.PeerID, ch <-chan []byte) {
	...
	for {
		...
		case msg, ok := <-ch:
			...
			sp.recvCh <- Message{Sender: pid, Payload: msg}
		}
	}
}
