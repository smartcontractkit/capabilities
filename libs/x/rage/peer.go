...
func NewPeer(cfg PeerConfig, lggr logger.Logger) (*peer, error) {
	...
	peerKeyring, err := NewSignerPeerKeyring(cfg.PrivateKey)
	if err != nil {
		return nil, err
	}
...
func (p *peer) UpdateConnections(peers map[ragetypes.PeerID]StreamConfig) error {
...
func (p *peer) recreateStreams(peers map[ragetypes.PeerID]StreamConfig) error {
...
func (p *peer) recvLoopSingle(pid ragetypes.PeerID, ch <-chan []byte) {
	...
	for {
		...
		case msg, ok := <-ch:
			...
			p.recvCh <- Message{Sender: pid, Payload: msg}
		}
	}
}
...
func (p *peer) Receive() <-chan Message {
	return p.recvCh
}
