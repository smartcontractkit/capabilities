...
func testClient(t *testing.T, numWorkflowPeers int, workflowNodeResponseTimeout time.Duration, numCapabilityPeers int, capabilityDonF uint8, underlying commoncap.ExecutableCapability, method func(caller commoncap.ExecutableCapability)) {
	...
	receivers := make([]don2dontypes.Receiver, numCapabilityPeers)
	for i := range numCapabilityPeers {
...
type clientTestServer struct {
	...
	workflowDonInfo    commoncap.DON
	messageIDToSenders map[string]map[rage.PeerID]bool
...
func newTestServer(peerID rage.PeerID, dispatcher don2dontypes.Dispatcher, workflowDonInfo commoncap.DON,
	...
	return &clientTestServer{
		...
		peerID:               peerID,
		messageIDToSenders:   make(map[string]map[rage.PeerID]bool),
		executableCapability: executableCapability,
	}
}

func (t *clientTestServer) Receive(ctx context.Context, msg *don2dontypes.MessageBody) {
	t.mux.Lock()
	defer t.mux.Unlock()
	...
	if t.messageIDToSenders[messageID] == nil {
		t.messageIDToSenders[messageID] = make(map[rage.PeerID]bool)
	}
	...
	if len(t.messageIDToSenders[messageID]) == len(t.workflowDonInfo.Members) {
		switch msg.Method {
		case don2dontypes.MethodExecute:
...
func (t *clientTestServer) sendResponse(messageID string, responseErr error,
	...
	for receiver := range t.messageIDToSenders[messageID] {
		var responseMsg = &don2dontypes.MessageBody{
		...
		if responseErr != nil {
			responseMsg.Error = don2dontypes.Error_INTERNAL_ERROR
		} else {
			if marshalErr != nil {
				panic(marshalErr)
			}
...
func TestClient_SetConfig(t *testing.T) {
	...
	t.Run("successful config update", func(t *testing.T) {
		...
		newDonInfo := commoncap.DON{
			ID:      2,
			Members: []rage.PeerID{NewP2PPeerID(t), NewP2PPeerID(t)},
			F:       1,
		}
