...
type ClientRequest struct {
	...
	responseReceived         map[rage.PeerID]bool
	lggr                     logger.Logger
	signers                  [][]byte
...
func newClientRequest(ctx context.Context, lggr logger.Logger, requestID string, remoteCapabilityInfo commoncap.CapabilityInfo,
	...
	responseReceived := make(map[rage.PeerID]bool)
	...
	for peerID, delay := range peerIDToTransmissionDelay {
		...
		wg.Add(1)
		go func(innerCtx context.Context, peerID ragetypes.PeerID, delay time.Duration) {
			defer wg.Done()
...
func emitTransmissionScheduleEvent(ctx context.Context, scheduleType, workflowExecutionID, transmissionID, capabilityID, stepRef string, peerIDToTransmissionDelay map[rage.PeerID]time.Duration) error {
...
func (c *ClientRequest) OnMessage(_ context.Context, msg *types.MessageBody) error {
	...
	sender, err := don2don.ToPeerID(msg.Sender)
	if err != nil {
...
func (c *ClientRequest) sendResponse(response clientResponse) {
	...
	c.respSent = true
	if response.Err != nil {
		c.lggr.Warnw("received error response", "error", don2don.SanitizeLogString(response.Err.Error()))
		return
	}
