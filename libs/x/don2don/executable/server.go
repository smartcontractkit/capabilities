...
import (
	...
	"github.com/smartcontractkit/capabilities/libs/x/don2don/executable/request"
...
func NewServer(capabilityID, methodName string, peerID rage.PeerID, dispatcher types.Dispatcher, workflowDONBindingGate limits.GateLimiter, lggr logger.Logger) *server {
	return &server{
...
func (r *server) Start(ctx context.Context) error {
	return r.StartOnce(r.Name(), func() error {
		...
		r.parallelExecutor = don2don.NewParallelExecutor(int(cfg.remoteExecutableConfig.ServerMaxParallelRequests), "executable_server")
...
func (r *server) Receive(ctx context.Context, msg *types.MessageBody) {
	...
	default:
		r.lggr.Errorw("received request for unsupported method type", "method", don2don.SanitizeLogString(msg.Method))
		return
	}

	messageID, err := GetMessageID(msg)
	if err != nil {
		r.lggr.Errorw("invalid message id", "err", err, "id", don2don.SanitizeLogString(string(msg.MessageId)))
		return
	}
