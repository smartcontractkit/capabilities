...
type testAsyncMessageBroker struct {
	...
	nodes map[rage.PeerID]don2dontypes.Receiver

	sendCh chan *don2dontypes.MessageBody
}

func newTestAsyncMessageBroker(t *testing.T, sendChBufferSize int) *testAsyncMessageBroker {
	b := &testAsyncMessageBroker{
		t:      t,
		nodes:  make(map[rage.PeerID]don2dontypes.Receiver),
		sendCh: make(chan *don2dontypes.MessageBody, sendChBufferSize),
	}
...
func (a *testAsyncMessageBroker) NewDispatcherForNode(nodePeerID rage.PeerID) don2dontypes.Dispatcher {
...
func (a *testAsyncMessageBroker) RegisterReceiverNode(nodePeerID rage.PeerID, node don2dontypes.Receiver) {
...
func (a *testAsyncMessageBroker) Send(msg *don2dontypes.MessageBody) {
	a.sendCh <- msg
}

func toPeerID(id []byte) rage.PeerID {
	return [32]byte(id)
}

type broker interface {
	Send(msg *don2dontypes.MessageBody)
}

type nodeDispatcher struct {
	callerPeerID rage.PeerID
	broker       broker
}
...
func (t *nodeDispatcher) Send(peerID rage.PeerID, msgBody *don2dontypes.MessageBody) error {
	msgBody.Version = 1
...
func (t *nodeDispatcher) SetReceiver(capabilityID string, donID uint32, receiver don2dontypes.Receiver) error {
	return nil
}
func (t *nodeDispatcher) RemoveReceiver(capabilityID string, donID uint32) {}

func (t *nodeDispatcher) SetReceiverForMethod(capabilityID string, donID uint32, methodName string, receiver don2dontypes.Receiver) error {
	return nil
}
...
func NewP2PPeerID(t *testing.T) rage.PeerID {
	id := rage.PeerID{}
