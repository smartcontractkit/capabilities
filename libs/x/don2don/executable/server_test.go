	...
	synctest.Test(t, func(t *testing.T) {
		...
		for _, caller := range callers {
			for range numCapabilityPeers {
				...
				assert.Equal(t, don2dontypes.Error_OK, msg.Error)
...
func Test_Server_InsufficientCallers(t *testing.T) {
	...
	for _, caller := range callers {
		for range numCapabilityPeers {
			msg := <-caller.receivedMessages
			assert.Equal(t, don2dontypes.Error_TIMEOUT, msg.Error)
		}
	}
	closeServices(t, srvcs)
}
...
func Test_Server_CapabilityError(t *testing.T) {
	...
	for _, caller := range callers {
		for range numCapabilityPeers {
			msg := <-caller.receivedMessages
			assert.Equal(t, don2dontypes.Error_INTERNAL_ERROR, msg.Error)
		}
	}
	closeServices(t, srvcs)
}
...
func (r *v2WriteChainMessageHasher) Hash(ctx context.Context, msg *don2dontypes.MessageBody) ([32]byte, error) {
...
func testRemoteExecutableCapabilityServer(ctx context.Context, t *testing.T,
	...
	messageHasher don2dontypes.MessageHasher) ([]*serverTestClient, []services.Service) {
	lggr := logger.Test(t)
	...
	capabilityPeers := make([]rage.PeerID, numCapabilityPeers)
	...
	workflowPeers := make([]rage.PeerID, numWorkflowPeers)
	...
	capabilityNodes := make([]don2dontypes.Receiver, numCapabilityPeers)
...
type serverTestClient struct {
	...
	receivedMessages  chan *don2dontypes.MessageBody
	callerDonID       string
}

func (r *serverTestClient) Receive(_ context.Context, msg *don2dontypes.MessageBody) {
	r.receivedMessages <- msg
}

func newServerTestClient(peerID rage.PeerID, capabilityDonInfo commoncap.DON,
	dispatcher don2dontypes.Dispatcher) *serverTestClient {
	return &serverTestClient{peerID: peerID, dispatcher: dispatcher, capabilityDonInfo: capabilityDonInfo,
		receivedMessages: make(chan *don2dontypes.MessageBody, 100), callerDonID: "workflow-don"}
}
...
func (r *serverTestClient) Execute(ctx context.Context, req commoncap.CapabilityRequest) (<-chan commoncap.CapabilityResponse, error) {
	...
	messageID := don2dontypes.MethodExecute + ":" + req.Metadata.WorkflowExecutionID

	for _, node := range r.capabilityDonInfo.Members {
		message := &don2dontypes.MessageBody{
			CapabilityId:    "capability-id",
			CapabilityDonId: 1,
			CallerDonId:     2,
			Method:          don2dontypes.MethodExecute,
			Payload:         rawRequest,
...
func Test_Server_SetConfig(t *testing.T) {
	...
	workflowDONs := map[uint32]commoncap.DON{
		2: {
			ID:      2,
			Members: []rage.PeerID{NewP2PPeerID(t)},
			F:       0,
		},
	}
...
func Test_Server_SetConfig_DONMembershipChange(t *testing.T) {
	...
	synctest.Test(t, func(t *testing.T) {
		...
		workflowDONs := map[uint32]commoncap.DON{
			2: {
				ID:      2,
				Members: []rage.PeerID{workflowPeer1},
				F:       0,
			},
		}
		...
		newWorkflowDONs := map[uint32]commoncap.DON{
			2: {
				ID:      2,
				Members: []rage.PeerID{workflowPeer1, workflowPeer2},
				F:       0,
			},
		}
...
func Test_Server_Execute_WithConcurrentSetConfig(t *testing.T) {
	...
	peerID := NewP2PPeerID(t)
	capabilityPeers := []rage.PeerID{peerID}
		...
		go func(c *serverTestClient) {
			...
			for i := range expectedResponsesPerCaller {
				...
				case msg := <-c.receivedMessages:
					if msg.Error == don2dontypes.Error_OK {
						successCount.Add(1)
					} else {
						errorCount.Add(1)
					}
...
func Test_Server_DuplicateRequestRemainsDedupedPastRequestTimeout(t *testing.T) {
	...
	localDON := commoncap.DON{
		ID:      1,
		Members: []rage.PeerID{serverPeerID},
		F:       0,
	}
	workflowDONs := map[uint32]commoncap.DON{
		2: {
			ID:      2,
			Members: []rage.PeerID{senderPeerID},
			F:       0,
		},
	}
	...
	msg := &don2dontypes.MessageBody{
		...
		Method:          don2dontypes.MethodExecute,
		Payload:         rawRequest,
		MessageId:       []byte(don2dontypes.MethodExecute + ":exec-1"),
		Sender:          senderPeerID[:],
...
func (n *noopDispatcher) SetReceiver(string, uint32, don2dontypes.Receiver) error { return nil }

func (n *noopDispatcher) RemoveReceiver(string, uint32) {}

func (n *noopDispatcher) SetReceiverForMethod(string, uint32, string, don2dontypes.Receiver) error {
	return nil
}

func (n *noopDispatcher) RemoveReceiverForMethod(string, uint32, string) {}

func (n *noopDispatcher) Send(peerID rage.PeerID, msgBody *don2dontypes.MessageBody) error {
	n.mu.Lock()
	defer n.mu.Unlock()
