...
import (
	...
	"github.com/smartcontractkit/capabilities/libs/x/don2don/types/mocks"
	"github.com/smartcontractkit/capabilities/libs/x/internal/synctest"
...
func TestTriggerPublisher_ReceiveTriggerEvents_BatchingEnabled(t *testing.T) {
	...
	synctest.Test(t, func(t *testing.T) {
		...
		dispatcher.On("Send", peers[1], mock.Anything).Run(func(args mock.Arguments) {
			msg := args.Get(1).(*don2dontypes.MessageBody)
			require.Equal(t, capID, msg.CapabilityId)
			require.Equal(t, don2dontypes.MethodTriggerEvent, msg.Method)
			require.NotEmpty(t, msg.Payload)
			metadata := msg.Metadata.(*don2dontypes.MessageBody_TriggerEventMetadata)
...
func TestTriggerPublisher_SetConfig_Basic(t *testing.T) {
	...
	t.Run("returns error when underlying trigger capability is nil", func(t *testing.T) {
		...
		publisher := don2don.NewTriggerPublisher(capInfo.ID, "method", dispatcher, lggr)
...
func newServices(t *testing.T, capabilityDONID uint32, workflowDONID uint32, maxBatchSize uint32, batchCollectionPeriod time.Duration) (*testTrigger, don2dontypes.ReceiverService, *mocks.Dispatcher, []rage.PeerID) {
	t.Helper()
	lggr := logger.Test(t)
	ctx := t.Context()
	capInfo := commoncap.CapabilityInfo{
	...
	capDonInfo := commoncap.DON{
		ID:      capabilityDONID,
		Members: []rage.PeerID{peers[0]}, // peer 0 is in the capability DON
		F:       0,
	}
	workflowDonInfo := commoncap.DON{
		ID:      workflowDONID,
		Members: []rage.PeerID{peers[1]}, // peer 1 is in the workflow DON
		F:       0,
	}
	...
	publisher := don2don.NewTriggerPublisher(capInfo.ID, "", dispatcher, lggr)
...
func allowRegistrationChecks(dispatcher *mocks.Dispatcher) {
	dispatcher.On("Send", mock.Anything, mock.MatchedBy(func(m *don2dontypes.MessageBody) bool {
		return m.Method == don2dontypes.MethodTriggerRegistrationCheck
	})).Return(nil).Maybe()
}

func newRegisterTriggerMessage(t *testing.T, callerDonID uint32, sender rage.PeerID) *don2dontypes.MessageBody {
	...
	require.NoError(t, err)
	return &don2dontypes.MessageBody{
		Sender:      sender[:],
...
func newAckEventMessage(t *testing.T, eventID string, triggerID string, callerDonID uint32, sender rage.PeerID) *don2dontypes.MessageBody {
	return &don2dontypes.MessageBody{
		Sender:      sender[:],
		Method:      don2dontypes.MethodTriggerEventAck,
		CallerDonId: callerDonID,
		Metadata: &don2dontypes.MessageBody_TriggerEventMetadata{
...
func TestTriggerPublisher_ExplicitUnregister(t *testing.T) {
	...
	// Send unregister
	unregMsg := &don2dontypes.MessageBody{
		Sender:      peers[1][:],
		Method:      don2dontypes.MethodUnregisterTrigger,
...
func TestTriggerPublisher_SendsRegistrationChecks(t *testing.T) {
	...
	checkReceived := make(chan *don2dontypes.MessageBody, 10)
	dispatcher.On("Send", mock.Anything, mock.MatchedBy(func(m *don2dontypes.MessageBody) bool {
		return m.Method == don2dontypes.MethodTriggerRegistrationCheck
...
func TestTriggerPublisher_RegistrationChecksChunkByMaxBatchSize(t *testing.T) {
	...
	require.NoError(t, peers[0].UnmarshalText([]byte(peerID1)))
	require.NoError(t, peers[1].UnmarshalText([]byte(peerID2)))
	capDonInfo := commoncap.DON{ID: capabilityDONID, Members: []rage.PeerID{peers[0]}, F: 0}
	workflowDonInfo := commoncap.DON{ID: workflowDONID, Members: []rage.PeerID{peers[1]}, F: 0}
	...
	var mu sync.Mutex
	var chunkLens []int
	dispatcher.On("Send", peers[1], mock.MatchedBy(func(m *don2dontypes.MessageBody) bool {
		return m.Method == don2dontypes.MethodTriggerRegistrationCheck
	})).Run(func(args mock.Arguments) {
		msg := args.Get(1).(*don2dontypes.MessageBody)
		meta := msg.GetTriggerEventMetadata()
...
func TestTriggerPublisher_UnregisterValidatesSenderMembership(t *testing.T) {
	...
	peers := make([]rage.PeerID, 3)
	require.NoError(t, peers[0].UnmarshalText([]byte(peerID1)))
	require.NoError(t, peers[1].UnmarshalText([]byte(peerID2)))
	// peers[2] is a random peer not in any DON
	peers[2] = rage.PeerID{0xff}
...
func TestTriggerPublisher_UnregisterRequiresQuorum(t *testing.T) {
	...
	workflowDonInfo := commoncap.DON{
		ID:      workflowDONID,
		Members: []rage.PeerID{peers[1], peers[2], peers[3]},
		F:       1,
	}
	workflowDONs := map[uint32]commoncap.DON{
		workflowDonInfo.ID: workflowDonInfo,
	...
	for _, p := range []rage.PeerID{peers[1], peers[2], peers[3]} {
	...
	unregMeta := &don2dontypes.MessageBody_TriggerEventMetadata{
		TriggerEventMetadata: &don2dontypes.TriggerEventMetadata{
			WorkflowIds: []string{workflowID1},
			TriggerIds:  []string{"triggerA"},
		},
	}
	recvUnreg := func(sender rage.PeerID) {
		publisher.Receive(ctx, &don2dontypes.MessageBody{
			Sender:      sender[:],
			Method:      don2dontypes.MethodUnregisterTrigger,
			CallerDonId: workflowDONID,
			Metadata:    unregMeta,
...
func TestTriggerPublisher_UnregisterInvalidMetadata(t *testing.T) {
	...
	cases := []struct {
		name string
		meta *don2dontypes.MessageBody_TriggerEventMetadata
	}{
		{
			name: "nil metadata",
			meta: nil,
...
func TestTriggerPublisher_AckCacheCleanup(t *testing.T) {
	...
	workflowDonInfo := commoncap.DON{
		ID:      workflowDONID,
		Members: []rage.PeerID{peers[0], peers[1]},
		F:       0,
	}
	workflowDONs := map[uint32]commoncap.DON{
		workflowDonInfo.ID: workflowDonInfo,
	...
	triggerEventSent := make(chan rage.PeerID, 10)
	dispatcher.On("Send", mock.Anything, mock.Anything).Run(func(args mock.Arguments) {
		msg := args.Get(1).(*don2dontypes.MessageBody)
		if msg.Method == don2dontypes.MethodTriggerEvent {
			triggerEventSent <- args.Get(0).(rage.PeerID)
		}
	}).Return(nil).Maybe()
	...
	sentTo := make(map[rage.PeerID]bool)
	for range 2 {
		select {
...
func newRegisterTriggerMessageWithTriggerID(t *testing.T, callerDonID uint32, sender rage.PeerID, triggerID string) *don2dontypes.MessageBody {
	triggerRequest := commoncap.TriggerRegistrationRequest{
		TriggerID: triggerID,
		Metadata: commoncap.RequestMetadata{
			WorkflowID: workflowID1,
