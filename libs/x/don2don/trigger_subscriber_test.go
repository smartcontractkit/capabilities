...
import (
	...
	remoteMocks "github.com/smartcontractkit/capabilities/libs/x/don2don/types/mocks"
...
func TestTriggerSubscriber_RegisterAndReceive(t *testing.T) {
	...
	subscriber := don2don.NewTriggerSubscriber(capInfo.ID, "method", dispatcher, lggr)
...
func TestTriggerSubscriber_AckReplayOnDuplicateReceive(t *testing.T) {
	...
	dispatcher.On("Send", mock.Anything, mock.Anything).Run(func(args mock.Arguments) {
		if m, ok := args.Get(1).(*don2dontypes.MessageBody); ok && m.Method == don2dontypes.MethodTriggerEventAck {
			ackSendsTotal.Add(1)
		}
	}).Return(nil).Maybe()
...
func TestTriggerSubscriber_RegistrationCheck(t *testing.T) {
	...
	newSubscriber := func(t *testing.T) (subscriberSvc, *remoteMocks.Dispatcher) {
		...
		sub := don2don.NewTriggerSubscriber(capInfo.ID, "method", dispatcher, lggr)
		agg := aggregation.NewDefaultModeAggregator(1)
	...
	buildCheckMsg := func(workflowID, triggerID string) *don2dontypes.MessageBody {
		return &don2dontypes.MessageBody{
			Sender:      capDon.Members[0][:],
			Method:      don2dontypes.MethodTriggerRegistrationCheck,
	...
	t.Run("does not resend registration when trigger exists", func(t *testing.T) {
		...
		for _, call := range dispatcher.Calls {
			...
			msg := call.Arguments.Get(1).(*don2dontypes.MessageBody)
			require.NotEqual(t, don2dontypes.MethodRegisterTrigger, msg.Method,
	...
	t.Run("sends unregister when trigger missing with correct metadata", func(t *testing.T) {
		...
		for _, call := range dispatcher.Calls {
			...
			msg := call.Arguments.Get(1).(*don2dontypes.MessageBody)
			if msg.Method == don2dontypes.MethodUnregisterTrigger {
	...
	t.Run("sends unregister after trigger is unregistered locally", func(t *testing.T) {
		...
		dispatcher.Calls = nil
		sub.Receive(t.Context(), buildCheckMsg(workflowID1, "triggerA"))
		dispatcher.AssertCalled(t, "Send", mock.Anything, mock.MatchedBy(func(m *don2dontypes.MessageBody) bool {
			return m.Method == don2dontypes.MethodUnregisterTrigger
		}))
	})
	...
	t.Run("ignores check with mismatched WorkflowIds and TriggerIds lengths", func(t *testing.T) {
		...
		for _, tc := range []struct {
			...
			t.Run(tc.name, func(t *testing.T) {
				...
				malformedMsg := &don2dontypes.MessageBody{
					Sender:      capDon.Members[0][:],
					Method:      don2dontypes.MethodTriggerRegistrationCheck,
					CallerDonId: workflowDon.ID,
					Metadata: &don2dontypes.MessageBody_TriggerEventMetadata{
				...
				for _, call := range dispatcher.Calls {
					if call.Method == "Send" {
						t.Errorf("expected no dispatcher.Send for malformed check message, got method=%v",
							call.Arguments.Get(1).(*don2dontypes.MessageBody).Method)
					}
				}
			})
		}
	...
	t.Run("ignores check from unknown sender", func(t *testing.T) {
		...
		unknownPeer := rage.PeerID{0xaa}
		checkMsg := &don2dontypes.MessageBody{
			Sender:      unknownPeer[:],
			Method:      don2dontypes.MethodTriggerRegistrationCheck,
			CallerDonId: workflowDon.ID,
			Metadata: &don2dontypes.MessageBody_TriggerEventMetadata{
		...
		for _, call := range dispatcher.Calls {
			if call.Method == "Send" {
				msg := call.Arguments.Get(1).(*don2dontypes.MessageBody)
				require.NotEqual(t, don2dontypes.MethodRegisterTrigger, msg.Method, "should not re-register from unknown sender")
				require.NotEqual(t, don2dontypes.MethodUnregisterTrigger, msg.Method, "should not unregister from unknown sender")
			}
		}
	})
}
...
func newTriggerSubscriberForTest(
	...
	cfg *commoncap.RemoteTriggerConfig,
	regReq commoncap.TriggerRegistrationRequest,
) (don2don.TriggerSubscriber, commoncap.DON, <-chan commoncap.TriggerResponse) {
	t.Helper()
...
func buildTriggerEvent(t *testing.T, sender []byte) *don2dontypes.MessageBody {
	triggerEventValue, err := values.NewMap(triggerEvent1)
	require.NoError(t, err)
	...
	return &don2dontypes.MessageBody{
		Sender: sender,
		Method: don2dontypes.MethodTriggerEvent,
		Metadata: &don2dontypes.MessageBody_TriggerEventMetadata{
			TriggerEventMetadata: &don2dontypes.TriggerEventMetadata{
				WorkflowIds: []string{workflowID1},
			},
...
func buildTriggerEventWithTriggerID(t *testing.T, sender []byte, workflowID, triggerID, eventID string) *don2dontypes.MessageBody {
	triggerEventValue, err := values.NewMap(triggerEvent1)
	require.NoError(t, err)
	capResponse := commoncap.TriggerResponse{
		Event: commoncap.TriggerEvent{
