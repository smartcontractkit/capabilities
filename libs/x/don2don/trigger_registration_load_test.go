...
func (d *countingDispatcher) Send(_ rage.PeerID, msgBody *don2dontypes.MessageBody) error {
...
func (d *countingDispatcher) SetReceiver(_ string, _ uint32, _ don2dontypes.Receiver) error {
	return nil
}
func (d *countingDispatcher) RemoveReceiver(_ string, _ uint32) {}
func (d *countingDispatcher) SetReceiverForMethod(_ string, _ uint32, _ string, _ don2dontypes.Receiver) error {
	return nil
}
...
func generatePeers(t *testing.T, n int) []rage.PeerID {
	t.Helper()
	peers := make([]rage.PeerID, n)
	for i := range n {
...
func TestRegistrationTrafficVolume(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(fmt.Sprintf("N=%d_cap=%d_wf=%d", tc.nRegistrations, tc.capDonSize, tc.workflowDonSize), func(t *testing.T) {
			...
			subscriber := don2don.NewTriggerSubscriber(capInfo.ID, "LogTrigger", dispatcher, lggr)
			...
			for time.Now().Before(deadline) {
				if dispatcher.Count(don2dontypes.MethodRegisterTrigger) >= expectedSends {
					break
				}
			...
			registerCount := snap[don2dontypes.MethodRegisterTrigger]
	...
	for _, tc := range cases {
		t.Run(fmt.Sprintf("N=%d", tc.nRegistrations), func(t *testing.T) {
			...
			synctest.Test(t, func(t *testing.T) {
				...
				publisher := don2don.NewTriggerPublisher(capInfo.ID, "LogTrigger", dispatcher, lggr)
				...
				snap := dispatcher.Snapshot()
				checkCount := snap[don2dontypes.MethodTriggerRegistrationCheck]
...
func BenchmarkRegistrationProcessing(b *testing.B) {
	...
	capDonMembers := make([]rage.PeerID, 4)
	...
	workflowDonMembers := make([]rage.PeerID, 7)
	...
	regMsg := &don2dontypes.MessageBody{
		Sender:      sender[:],
		Method:      don2dontypes.MethodRegisterTrigger,
		CallerDonId: workflowDon.ID,
		Payload:     marshaled,
...
func TestRegistrationLoopLockDuration(t *testing.T) {
	...
	for _, n := range cases {
		t.Run(fmt.Sprintf("N=%d", n), func(t *testing.T) {
			...
			actual := dispatcher.Count(don2dontypes.MethodRegisterTrigger)
...
func TestTrafficAttribution_RegisterLoopVsChecksVsEventsAndAcks(t *testing.T) {
	...
	sub := don2don.NewTriggerSubscriber(capInfo.ID, "LogTrigger", subRegDisp, lggr)
	...
	for time.Now().Before(deadline) {
		if subRegDisp.Count(don2dontypes.MethodRegisterTrigger) >= perTickReg {
			break
		}
		time.Sleep(10 * time.Millisecond)
	}
	...
	sub2 := don2don.NewTriggerSubscriber(capInfo.ID, "LogTrigger", subAckDisp, lggr)
	...
	ackFanout := subAckDisp.Count(don2dontypes.MethodTriggerEventAck)
	...
	pub := don2don.NewTriggerPublisher(capInfo.ID, "LogTrigger", pubDisp, lggr)
	...
	for i := range nPublisherRegs {
		...
		for j := range minRegSenders {
			...
			pub.Receive(ctx, &don2dontypes.MessageBody{
				Sender:      wfPeers[j][:],
				Method:      don2dontypes.MethodRegisterTrigger,
	...
	phase3Checks := pubDisp.Count(don2dontypes.MethodTriggerRegistrationCheck)
	...
	pub2 := don2don.NewTriggerPublisher(capInfo.ID, "LogTrigger", pubEvDisp, lggr)
	...
	for j := range minRegSenders {
		...
		payload, mErr := pb.MarshalTriggerRegistrationRequest(req)
		require.NoError(t, mErr)
		pub2.Receive(ctx, &don2dontypes.MessageBody{
			Sender:      wfPeers[j][:],
			Method:      don2dontypes.MethodRegisterTrigger,
			CallerDonId: wfDon.ID,
			Payload:     payload,
	...
	afterEvent := pubEvDisp.Count(don2dontypes.MethodTriggerEvent)
	...
	for m, v := range afterAck {
		if m == don2dontypes.MethodTriggerEvent {
